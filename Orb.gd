...
enum OrbType {ATTACK, DEFENSE, PREPARE, QI}
...
func enter_state(new_state):
	...
	match(state):
		OrbState.FALLING:
			...
			tween.tween_property(self, "position", final_position, .25 * (final_position.y - position.y) / 64).set_ease(Tween.EASE_IN)
			tween.tween_callback(_on_fall_complete)
			board.add_active_orb(self)
		OrbState.SWAPPING:
			tween = create_tween()
			var final_position = 64*Vector2(coordinate.x,4-coordinate.y)
			tween.tween_property(self, "position", final_position, .5).set_ease(Tween.EASE_IN_OUT)
		...
		OrbState.ACTIVATED:
			tween = create_tween()
			tween.tween_property(self, "modulate", Color(0,0,0,0), .5).set_ease(Tween.EASE_IN_OUT)
...
func exit_state(old_state):
	match(old_state):
		...
		OrbState.SELECTED:
			pass
		OrbState.ACTIVATED:
			board.remove_active_orbs(self)
			board.clear_orb(coordinate)
			GameManager.resources[0][type] += 1
			queue_free()
